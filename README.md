...
| `http://<VPS_IP>:<端口>/epg.xml` | 标准 XMLTV 输出（给 Kodi / TVHeadend / Emby 做 EPG 源） |

### XMLTV 接口（播放器 EPG 源）

```bash
# 默认: 官方最大未来范围 (实时拉取官方 API, 当前约 8 天, 3 台 900 条)
http://<VPS_IP>:<端口>/epg.xml
# 指定未来 N 天 (官方最多约 8 天, 超出部分自动截断)
http://<VPS_IP>:<端口>/epg.xml?days=7
# 指定某一天 (该日期在未来且官方可查时走官方, 否则读本地归档)
http://<VPS_IP>:<端口>/epg.xml?date=2026-09-03
# 启用认证时需带凭据
http://admin:hugo2026+@<VPS_IP>:<端口>/epg.xml?days=7
```

> 输出为标准 XMLTV（`<tv>` 根节点 + `<channel>` 定义），含翡翠台 / J2 / 明珠台。
> 数据源优先官方 myTV SUPER API（最新最全），官方请求失败时自动回退本地归档。
